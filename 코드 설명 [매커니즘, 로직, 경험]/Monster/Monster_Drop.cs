...
//
// ※ 몬스터 제거(토벌 + 놓아주기) 시 플레이어가 획득 가능한 아이템을 관리하는 Monster_Drop 클래스는 다른 Monster_ㆍㆍㆍ 클래스와 마찬가지로 각종 몬스터의 ㆍㆍㆍ_Drop 클래스를 상속으로 구현했었다.
//    그러나 각각의 몬스터가 각각의 드랍 아이템 저장소를 가지는것은 비효율적이라 생각해 MonsterManager 클래스로 통합되었다.
