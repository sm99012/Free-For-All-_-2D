...
public class Slime2_Move : Monster_Move // 기반이 되는 Monster_Move 클래스 상속
{
    void Awake()
    {
    ...
    // 몬스터 추격 함수 - "큰 초원 슬라임"은 매우 느린 속도로 오브젝트(플레이어)를 추격한다.
    ...
    // 몬스터 공격 함수 - "큰 초원 슬라임"은 공격 시 몸을 뻗어 오브젝트(플레이어)와 충돌하는 공격을 한다.
            ...
            case E_MONSTER_MOVE_STATE.ATTACK:
                {
                    if (m_eMonsterState != ms)
                    {
                        if (m_eMonsterState == E_MONSTER_MOVE_STATE.CHASE) // 몬스터 동작 FSM 상태 판단
                        {
                            if (m_bAttack == true) // 몬스터 공격 가능
                            {
