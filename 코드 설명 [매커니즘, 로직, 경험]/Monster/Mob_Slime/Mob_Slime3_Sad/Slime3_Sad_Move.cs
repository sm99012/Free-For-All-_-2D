...
public class Slime3_Sad_Move : Slime3_Move // 기반이 되는 Slime3_Move 클래스 상속
{
    // 부모 클래스인 Slime3_Move Awake() 함수를 사용한다.
    // private void Awake() {ㆍㆍㆍ}

    // 부모 클래스인 Slime3_Move Start() 함수를 사용한다.
    // void Start() {ㆍㆍㆍ}

    // 몬스터 이동 함수 - 부모 클래스인 Slime3_Move Move() 함수를 사용한다.
    // override public void Move(int speed, Vector3 dir) {ㆍㆍㆍ}

    // 몬스터 추격 함수 - 부모 클래스인 Slime3_Move Chase() 함수를 사용한다.
    ...
    // 몬스터 방향 설정 함수 - 부모 클래스인 Slime3_Move SetDir() 함수를 사용한다.
    // override public void SetDir(Vector3 dir) {ㆍㆍㆍ}

    // 몬스터 공격 함수 - 부모 클래스인 Slime3_Move Attack() 함수를 사용한다.
    ...
    // 몬스터 피격 함수 - 부모 클래스인 Slime3_Move Attacked() 함수를 사용한다.
    ...
    // 몬스터 사망 함수. Fadeout 효과 관련 - 부모 클래스인 Slime3_Move Death() 함수를 사용한다.
    ...
    // 몬스터 놓아주기 함수 - 부모 클래스인 Slime3_Move Goaway() 함수를 사용한다.
    ...
    // 몬스터 동작 FSM 변경 함수 - 부모 클래스인 Slime3_Move SetMonsterMoveState() 함수를 사용한다.
    // override public E_MONSTER_MOVE_STATE SetMonsterMoveState(E_MONSTER_MOVE_STATE ms, float attackspeed = 0) {ㆍㆍㆍ}

    // 애니메이션 관리 - 부모 클래스인 Slime3_Move SetAnimationParameters() 함수를 사용한다.
