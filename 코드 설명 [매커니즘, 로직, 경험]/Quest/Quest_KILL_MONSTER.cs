...
public class Quest_KILL_MONSTER : Quest // 기반이 되는 Quest 클래스 상속
{
    ...
    public List<int> m_nl_Count_Max;     // 특정 몬스터 토벌 필요 수량 리스트
    public List<int> m_nl_Count_Current; // 특정 몬스터 토벌 현재 수량 리스트

    //
    // ※ m_nl_MonsterCode(특정 몬스터 고유코드 리스트), m_nl_Count_Max(특정 몬스터 토벌 필요 수량 리스트), m_nl_Count_Current(특정 몬스터 토벌 현재 수량 리스트)는 리스트 순번에 따라 짝을 이룬다.
    ...
    // 특정 몬스터 토벌 퀘스트 갱신 함수
    // return true : 플레이어가 토벌한 몬스터가 해당 특정 몬스터 토벌 퀘스트와 관련 있음 / return false : 플레이어가 토벌한 몬스터가 해당 특정 몬스터 토벌 퀘스트와 관련 없음
    public bool Check_KILL_MONSTER(int monstercode) // monstercode : 플레이어가 토벌한 몬스터 고유코드
    {
    ...
    override public void Check_Condition()
    {
        for (int i = 0; i < m_nl_MonsterCode.Count; i++) // 특정 몬스터 수량 만큼 반복
        {
            if (m_nl_Count_Max[i] == m_nl_Count_Current[i]) // 특정 몬스터 토벌 필요 수량 == 특정 몬스터 토벌 현재 수량 : 퀘스트 완료 조건 충족
            {
                m_bCondition = true;
                continue;
            }
            else if (m_nl_Count_Max[i] > m_nl_Count_Current[i]) // 특정 몬스터 토벌 필요 수량 > 특정 몬스터 토벌 현재 수량 : 퀘스트 완료 조건 미달
            {
                m_bCondition = false;
                break;
            }
