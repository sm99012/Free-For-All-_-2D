...
//
// ※ 특정 몬스터 놓아주기 퀘스트 클래스
//    해당 퀘스트는 특정 몬스터 놓아주기 행위와 연관되어 있다.
//

public class Quest_GOAWAY_MONSTER : Quest // 기반이 되는 Quest 클래스 상속
{
    // 특정 몬스터 관련 변수
    public List<int> m_nl_MonsterCode;   // 특정 몬스터 고유코드 리스트
    public List<int> m_nl_Count_Max;     // 특정 몬스터 놓아주기 필요 수량 리스트
    public List<int> m_nl_Count_Current; // 특정 몬스터 놓아주기 현재 수량 리스트

    //
    // ※ m_nl_MonsterCode(특정 몬스터 고유코드 리스트), m_nl_Count_Max(특정 몬스터 놓아주기 필요 수량 리스트), m_nl_Count_Current(특정 몬스터 놓아주기 현재 수량 리스트)는 리스트 순번에 따라 짝을 이룬다.
    //    m_nl_MonsterCode[n]에 해당하는 고유코드를 가진 몬스터를 m_nl_Count_Max[n]만큼 놓아주기 해야 한다. 이때 m_nl_Count_Current[n]을 통해 놓아주기 현황을 확인할 수 있다.
    //

    // 특정 몬스터 놓아주기 퀘스트를 생성하는 생성자
    ...
    // 특정 몬스터 놓아주기 퀘스트 설정(추가) 함수
    public void AddCondition(int monstercode, int max, int c = 0) // monstercode : 특정 몬스터 고유코드, max : 특정 몬스터 놓아주기 필요 수량, c : 특정 몬스터 놓아주기 현재 수량
    {
    ...
    // 특정 몬스터 놓아주기 퀘스트 갱신 함수
    // return true : 플레이어가 놓아주기한 몬스터가 해당 특정 몬스터 놓아주기 퀘스트와 관련 있음 / return false : 플레이어가 놓아주기한 몬스터가 해당 놓아주기 몬스터 토벌 퀘스트와 관련 없음
    ...
    override public void Check_Condition()
    {
        for (int i = 0; i < m_nl_MonsterCode.Count; i++) // 특정 몬스터 수량만큼 반복
        {
            ...
            else if (m_nl_Count_Max[i] > m_nl_Count_Current[i]) // 특정 몬스터 놓아주기 필요 수량 > 특정 몬스터 놓아주기 현재 수량 : 퀘스트 완료 조건 충족
            {
                m_bCondition = false;
                break;
            }
