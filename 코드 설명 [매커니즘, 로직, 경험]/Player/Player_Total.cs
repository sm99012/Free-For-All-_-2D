...
public class Player_Total : MonoBehaviour
{
    ...
    // 플레이어가 착용한 무기 분류에 따라 상이한 공격 패턴(공격범위, 데미지 계수, 공격 타이밍)을 가진다.
    // Sword(검) 공격범위
    ...
    // 기본 공격 애니메이션의 특정 프레임에서 호출된다. 기본 공격의 종류별로 공격범위, 데미지 계수, 공격 타이밍이 다르다. 
    // Sword(검) 기본 공격 판정
    public void AttackCheck1_1_Sword()
    {
        AttackCheck(E_ITEM_EQUIP_MAINWEAPON_TYPE.SWORD, 1, 1f); // 기본 공격1 판정. 데미지 계수 : 100%
    }
    public void AttackCheck1_2_Sword()
    {
        AttackCheck(E_ITEM_EQUIP_MAINWEAPON_TYPE.SWORD, 2, 2f); // 기본 공격2 판정. 데미지 계수 : 200%
    }
    public void AttackCheck1_3_Sword()
    {
        AttackCheck(E_ITEM_EQUIP_MAINWEAPON_TYPE.SWORD, 3, 4f); // 기본 공격3 판정. 데미지 계수 : 400%
    }
    // Axe(도끼) 기본 공격 판정
    public void AttackCheck1_1_Axe()
    {
        AttackCheck(E_ITEM_EQUIP_MAINWEAPON_TYPE.AXE, 1, 1.5f); // 기본 공격1 판정. 데미지 계수 : 150%
    }
    public void AttackCheck1_2_Axe()
    {
        AttackCheck(E_ITEM_EQUIP_MAINWEAPON_TYPE.AXE, 2, 2f); // 기본 공격2 판정. 데미지 계수 : 200%
    }
    public void AttackCheck1_3_Axe()
    {
        AttackCheck(E_ITEM_EQUIP_MAINWEAPON_TYPE.AXE, 3, 6f); // 기본 공격3 판정. 데미지 계수 : 600%
    }
    // Knife(단검) 기본 공격 판정
    public void AttackCheck1_1_Knife()
    {
        AttackCheck(E_ITEM_EQUIP_MAINWEAPON_TYPE.KNIFE, 1, 1f); // 기본 공격1 판정. 데미지 계수 : 100%
    }
    public void AttackCheck1_2_Knife()
    {
        AttackCheck(E_ITEM_EQUIP_MAINWEAPON_TYPE.KNIFE, 2, 1.5f); // 기본 공격2 판정. 데미지 계수 : 150%
    }
    public void AttackCheck1_3_Knife()
    {
        AttackCheck(E_ITEM_EQUIP_MAINWEAPON_TYPE.KNIFE, 3, 2.5f); // 기본 공격3 판정. 데미지 계수 : 250%
    }

    // 기본 공격 판정 함수
    // 오버랩을 이용해 공격 범위내의 모든 오브젝트(몬스터, 파괴 가능한 오브젝트)에 특정 데미지 계수를 적용한 데미지를 가한다.
    ...
    void AttackCheck(E_ITEM_EQUIP_MAINWEAPON_TYPE mt, int attacknumber, float percent) // mt : 무기 분류, attacknumber : 기본 공격 단계(1, 2, 3), percent : 데미지 계수
    {
