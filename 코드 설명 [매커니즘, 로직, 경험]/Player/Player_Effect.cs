...
public class Player_Effect : MonoBehaviour
{
    ...
    public GameObject m_gCondition_Bind;    // 속박 이펙트(애니메이션) 10000
    public GameObject m_gCondition_Shock;   // 기절 이펙트(애니메이션) 11000
    public GameObject m_gCondition_Dark;    // 암흑 이펙트(애니메이션) 12000
    public GameObject m_gCondition_Slow;    // 둔화 이펙트(애니메이션) 13000
    public GameObject m_gCondition_Confuse; // 혼란 이펙트(애니메이션) 14000
    ...
    public bool ApplySkill(int ncode, Skill_Effect ske) // ncode : 적용할 스킬코드, skse : 스킬 이펙트
    {
        if (m_sDictionary_Skill_Effect_Apply.ContainsKey(ncode) == false)
            m_sDictionary_Skill_Effect_Apply.Add(ncode, ske);

        ApplySkill_Condition_Effect();
    ...
    public void UnApplySkill_Condition_Effect(int ncode) // ncode : 해제할 스킬(상태이상)코드
    {
        ...
        if (ncode / 1000 == 12)
            m_gCondition_Dark.SetActive(false);    // 상태이상(암흑) 이팩트 비활성화
        if (ncode / 1000 == 13)
